...
import { fetchKaitWalletSummary } from '@/store/slices/user/walletSummary/kaitWalletSummarySlice'
...
const KaitWalletSummary = () => {
  ...
  const { items, total, total_pages, loading, error } = useAppSelector(
    (state) => state.KaitwalletSummary,
  )

  useEffect(() => {
    dispatch(fetchKaitWalletSummary({ page, search }))
