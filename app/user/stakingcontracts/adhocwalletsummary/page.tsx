...
import { fetchAdhocWalletSummary } from '@/store/slices/user/walletSummary/adhocWalletSummarySlice'
...
const AdhocWalletSummary = () => {
  ...
  const { items, total, total_pages, loading, error } = useAppSelector(
    (state) => state.AdhocwalletSummary,
  )

  useEffect(() => {
    dispatch(fetchAdhocWalletSummary({ page, search }))
