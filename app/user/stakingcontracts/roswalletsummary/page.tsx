...
import { fetchRosWalletSummary } from '@/store/slices/user/walletSummary/rosWalletSummarySlice'
...
const RosWalletSummary = () => {
  ...
  const { items, total, total_pages, loading, error } = useAppSelector(
    (state) => state.RoswalletSummary,
  )

  useEffect(() => {
    dispatch(fetchRosWalletSummary({ page, search }))
