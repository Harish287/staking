...
const KycDetails = ({ params }: { params: { id: string } }) => {
  ...
  const kycApplications = useSelector((state: RootState) => state.auth?.kycApplications ?? []);
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen flex items-center justify-center">
      <Card className="p-6 shadow-xl rounded-xl bg-white w-[80%]">
        ...
        <p><strong>Username:</strong> {kycData.user_name}</p>
        <p><strong>User ID:</strong> {kycData.user_id}</p>
        <p><strong>Document Type:</strong> {kycData.doc_type}</p>
        <p><strong>Submitted On:</strong> {kycData.submitted}</p>
