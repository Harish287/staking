...
import { fetchIncomeWalletSummary } from '@/store/slices/user/walletSummary/incomeWalletSummarySlice'
...
const IncomeWalletSummary = () => {
  ...
  const { items, total, total_pages, loading, error } = useAppSelector(
    (state) => state.IncomewalletSummary,
  )

  useEffect(() => {
    dispatch(fetchIncomeWalletSummary({ page, search }))
